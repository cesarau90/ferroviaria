...
import { Activity, AlertTriangle, Bell, BookOpen, Box, Calendar, ChevronRight, CircleDot, Cpu, Gauge, ListFilter, Lock, LogOut, Map as MapIcon, MapPin, Play, Plus, Radio, RotateCcw, ShieldCheck, SlidersHorizontal, Train, Unlock, Users, Wifi, X } from 'lucide-react'
...
function Trips({user}:{user:User}){
  ...
  const load=()=>request<Trip[]>('/trips').then(setTrips)
  useEffect(()=>{
    void load()
    const refresh=()=>void load()
  ...
  const startTrip=async(e:React.MouseEvent,id:number)=>{
    e.preventDefault();e.stopPropagation();setNotice('');setBusyId(id)
    try{await request(`/trips/${id}/start`,{method:'POST'});setNotice('Viaje iniciado. La telemetría en vivo comenzará en segundos.');await load()}
  ...
  const resetTrip=async(e:React.MouseEvent,id:number)=>{
    ...
    setNotice('');setBusyId(id)
    try{await request(`/trips/${id}/reset`,{method:'POST'});setNotice('Demostración reiniciada correctamente.');await load()}
    catch(err){setNotice((err as Error).message)}
    finally{setBusyId(null)}
  }
  return <>
    ...
    {notice && <p className="mb-4 rounded-lg bg-cyan-500/10 p-3 text-sm text-cyan-200">{notice}</p>}
...
function ControlCenter({user}:{user:User}){
  ...
  const load=()=>request<any>(`/trips/${id}`).then(setData)
  ...
  const startTrip=async()=>{setNotice('');setBusy(true);try{await request(`/trips/${id}/start`,{method:'POST'});setNotice('Viaje iniciado.');await load()}catch(e){setNotice((e as Error).message)}finally{setBusy(false)}}
  const resetTrip=async()=>{
    if(!window.confirm('¿Reiniciar la demostración de este viaje? Se restablecerá el progreso, la posición, la batería, los candados/compuertas y se resolverán las alertas activas.')) return
    setNotice('');setBusy(true);try{await request(`/trips/${id}/reset`,{method:'POST'});setNotice('Demostración reiniciada.');await load()}catch(e){setNotice((e as Error).message)}finally{setBusy(false)}
  }
  return <>
