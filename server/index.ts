...
app.post('/api/alerts/:id/archive', token, allow('ADMIN','OPERATOR'), (req,res) => { const alertId=Number(req.params.id); const a=db.prepare('SELECT * FROM alerts WHERE id=?').get(alertId) as any; if(!a) return res.status(404).json({message:'Alert not found'}); if(a.status==='ACTIVE') return res.status(400).json({message:'No se puede archivar una alerta activa.'}); db.prepare("UPDATE alerts SET status='ARCHIVED' WHERE id=?").run(alertId); audit((req as any).user.id,'ALERT_ARCHIVED',null,null,'SUCCESS',`Alert ${alertId}`); io.emit('alert:update',{id:alertId,status:'ARCHIVED'}); res.json({ok:true}) })
